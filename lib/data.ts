...
export async function getParejas(grupo?: Grupo): Promise<Pareja[]> {
  ...
  const { data } = await q
  return (data ?? []).map((p) => ({ ...p, nombre: `${p.jugador1} - ${p.jugador2}` }))
}
...
export async function getPartidos(fechaId?: number): Promise<Partido[]> {
  ...
  return (data ?? []).map((p) => ({
    ...p,
    pareja1: p.pareja1 ? { ...p.pareja1, nombre: `${p.pareja1.jugador1} - ${p.pareja1.jugador2}` } : undefined,
    pareja2: p.pareja2 ? { ...p.pareja2, nombre: `${p.pareja2.jugador1} - ${p.pareja2.jugador2}` } : undefined,
  }))
}
